/**
 * Iowa Superintendent Data - Load from CSV
 *
 * Source: Iowa Department of Education Public School District Directory
 * URL: https://educate.iowa.gov/iowa-public-school-district-directories
 * NCES: ~330 districts
 * CSV: 325 school districts with superintendents
 *
 * Note: Complete coverage of Iowa public school districts.
 * Source Excel file includes superintendent name, email, and phone.
 */
...
const SOURCE_URL = 'https://educate.iowa.gov/iowa-public-school-district-directories';
...
function normalizeDistrictName(name) {
  return name
    .toLowerCase()
    .trim()
    .replace(/comm school district/gi, '')
    .replace(/community school district/gi, '')
...
async function main() {
  ...
  try {
    ...
    if (parseInt(existingResult.rows[0].count) > 0) {
      console.log(`\nWARNING: IA already has ${existingResult.rows[0].count} records`);
    ...
    const csvPath = path.join(__dirname, '../data/processed/ia_superintendents.csv');
    ...
    for (const line of lines) {
      ...
      records.push({
        ...
        state_district_id: district.replace(/Comm School District|Community School District|School District/gi, '').trim(),
    ...
    console.log(`Found ${ncesResult.rows.length} NCES districts for IA`);
    ...
    const importResult = await client.query(`
      ...
      VALUES (gen_random_uuid(), 'state_registry', 'Iowa DOE Public School District Directory', $1, 'ia_superintendents.csv', $2,
              NOW(), 'fetch-load-ia.js', 'IA districts from Iowa DOE 2024-25 directory. Complete with emails and phones.')
      RETURNING id
    ...
    for (const rec of records) {
      ...
      if (bestMatch) {
        await client.query(`
          ...
          VALUES (gen_random_uuid(), $1, $2, $3, $4, NOW(), 'fetch-load-ia.js', $5, $6)
    ...
    const coverageResult = await client.query(`
      ...
      FROM national_registry WHERE state = 'IA'
    `);
    const coverage = coverageResult.rows[0];
    console.log(`\nIA Coverage: ${coverage.with_supt}/${coverage.total} (${coverage.pct}%)`);
