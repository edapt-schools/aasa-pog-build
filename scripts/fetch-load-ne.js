/**
 * Nebraska Superintendent Data - Load from NDE Excel
 *
 * Source: Nebraska Department of Education Public Schools Administrators
 * URL: https://www.education.ne.gov/nebraska-public-schools-administrators-email-lists/
 * NCES: ~278 districts
 * Excel: ~242 superintendents
 *
 * Complete data with superintendent names, emails, phones, and addresses.
 */
...
const SOURCE_URL = 'https://www.education.ne.gov/nebraska-public-schools-administrators-email-lists/';
...
async function main() {
  ...
  try {
    ...
    if (parseInt(existingResult.rows[0].count) > 0) {
      console.log(`\nWARNING: NE already has ${existingResult.rows[0].count} records`);
      console.log('Skipping to avoid duplicates. Run delete-state.js NE first.');
      await client.end();
      return;
    }

    console.log('=== READING XLSX ===');
    const xlsxPath = path.join(__dirname, '../data/raw/ne_superintendents.xlsx');
    ...
    for (const row of data) {
      ...
      records.push({
        state: 'NE',
        district_name: titleCase(row.Company),
        state_district_id: row.AgencyID,
        administrator_first_name: titleCase(row.FirstName),
        administrator_last_name: titleCase(row.LastName),
        administrator_email: row.EmailAddress?.toLowerCase(),
        phone: row.BusinessPhone,
        address: row.Address,
        city: titleCase(row.City),
        website_url: extractWebsite(row.EmailAddress)
      });
    }

    console.log(`Parsed ${records.length} superintendent records`);
    ...
    console.log(`Found ${ncesResult.rows.length} NCES districts for NE`);
    ...
    const importResult = await client.query(`
      ...
      VALUES (gen_random_uuid(), 'state_registry', 'NDE Public Schools Administrators', $1, 'ne_superintendents.xlsx', $2,
              NOW(), 'fetch-load-ne.js', 'NE superintendents from NDE. Complete with names, emails, phones, addresses.')
      RETURNING id
    ...
    for (const rec of records) {
      ...
      if (bestMatch) {
        await client.query(`
          ...
          VALUES (gen_random_uuid(), $1, $2, $3, $4, NOW(), 'fetch-load-ne.js', $5, $6)
    ...
    const coverageResult = await client.query(`
      ...
      FROM national_registry WHERE state = 'NE'
    `);
    const coverage = coverageResult.rows[0];
    console.log(`\nNE Coverage: ${coverage.with_supt}/${coverage.total} (${coverage.pct}%)`);
