/**
 * DC Superintendent Data - Load from CSV
 *
 * Source: DCPS Core Leadership / OSSE
 * URL: https://dcps.dc.gov/page/core-leadership-team
 * NCES: ~71 districts (1 DCPS + ~70 charter LEAs)
 * CSV: 1 record (DCPS only)
 *
 * Note: DC has DCPS as the main public school system plus ~70 charter LEAs.
 * Charter LEA leaders are not consolidated in a single directory and would
 * require visiting each charter school's website individually.
 */

const { Client } = require('pg');

const DATABASE_URL = 'postgresql://postgres:UMK-egr6gan5vdb.nzx@db.wdvpjyymztrebwaiaidu.supabase.co:5432/postgres';
const SOURCE_URL = 'https://dcps.dc.gov/page/core-leadership-team';
...
function normalizeDistrictName(name) {
  return name
    ...
    .replace(/district of columbia/gi, 'dc')
    .replace(/[#.,()]/g, '')
    .replace(/\s+/g, ' ')
    .trim();
}
...
async function main() {
  ...
  try {
    ...
    if (parseInt(existingResult.rows[0].count) > 0) {
      console.log(`\nWARNING: DC already has ${existingResult.rows[0].count} records`);
      console.log('Skipping to avoid duplicates.');
      await client.end();
      return;
    }

    // DC has one main LEA: DCPS
    const records = [{
      state: 'DC',
      district_name: 'District of Columbia Public Schools',
      state_district_id: 'DCPS',
      administrator_first_name: 'Lewis D.',
      administrator_last_name: 'Ferebee',
      administrator_email: 'lewis.ferebee@dc.gov',
      phone: '(202) 442-5885',
      website_url: 'https://dcps.dc.gov'
    }];
    ...
    console.log(`Found ${ncesResult.rows.length} NCES districts for DC`);
    ...
    const importResult = await client.query(`
      ...
      VALUES (gen_random_uuid(), 'state_registry', 'DCPS Core Leadership', $1, 'dc_superintendents.csv', $2,
              NOW(), 'fetch-load-dc.js', 'DC DCPS only. ~70 charter LEAs not included - no consolidated directory available.')
      RETURNING id
    ...
    for (const rec of records) {
      ...
      if (bestMatch) {
        await client.query(`
          ...
          VALUES (gen_random_uuid(), $1, $2, $3, $4, NOW(), 'fetch-load-dc.js', $5, $6)
    ...
    const coverageResult = await client.query(`
      ...
      FROM national_registry WHERE state = 'DC'
    `);
    const coverage = coverageResult.rows[0];
    console.log(`\nDC Coverage: ${coverage.with_supt}/${coverage.total} (${coverage.pct}%)`);
    console.log(`Note: DC has ~70 charter LEAs not included in this load.`);
