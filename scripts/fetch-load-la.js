/**
 * Louisiana Superintendent Data - Load from CSV
 *
 * Source: Louisiana Association of Superintendents and School Administrators (LASSA)
 * URL: https://louisianalassa.org/member-districts
 * NCES: 71 districts (includes special districts, charter authorities)
 * CSV: 69 parish/city school boards
 *
 * Note: Louisiana uses parishes instead of counties.
 * The 2-district gap consists of:
 * - Special School District (state-operated)
 * - Other non-traditional entities
 * All 69 parish and city districts are complete.
 */
...
const SOURCE_URL = 'https://louisianalassa.org/member-districts';
...
async function main() {
  ...
  try {
    ...
    if (parseInt(existingResult.rows[0].count) > 0) {
      console.log(`\nWARNING: LA already has ${existingResult.rows[0].count} records in state_registry_districts`);
    ...
    const csvPath = path.join(__dirname, '../data/processed/la_superintendents.csv');
    ...
    for (const line of lines) {
      ...
      // Extract parish/city name as state_district_id
      let stateDistrictId = district
        .replace(/School Board|School System|Public Schools?/gi, '')
        .trim();
    ...
    console.log(`Found ${ncesResult.rows.length} NCES districts for LA`);
    console.log('Note: Includes special districts and charter authorities');
    ...
    const importResult = await client.query(`
      ...
      VALUES (gen_random_uuid(), 'state_registry', 'LASSA Member Districts', $1, 'la_superintendents.csv', $2,
              NOW(), 'fetch-load-la.js', 'LA 69 parish/city school boards from LASSA. Gap = special districts.')
      RETURNING id
    ...
    for (const rec of records) {
      ...
      if (bestMatch) {
        await client.query(`
          ...
          VALUES (gen_random_uuid(), $1, $2, $3, $4, NOW(), 'fetch-load-la.js', $5, $6)
        `, [
          bestMatch.nces_id,
          stateRegistryId,
          matchMethod,
    ...
    console.log(`\nLA Coverage: ${coverage.with_supt}/${coverage.total} (${coverage.pct}%)`);
