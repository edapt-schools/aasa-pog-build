/**
 * Wyoming Superintendent Data - Load from CSV
 *
 * Source: Wyoming DOE School Directory
 * URL: https://edu.wyoming.gov/
 * NCES: 61 districts (includes BOCES, institutions, charters)
 * CSV: 48 traditional county school districts
 *
 * Note: The 13-district gap consists of:
 * - BOCES (educational cooperatives): 3
 * - Children's homes/institutions: 4
 * - State facilities: 3
 * - Charter/alternative schools: 3
 * Traditional county districts are complete.
 */
...
const SOURCE_URL = 'https://edu.wyoming.gov/';
...
async function main() {
  ...
  try {
    ...
    if (parseInt(existingResult.rows[0].count) > 0) {
      console.log(`\nWARNING: WY already has ${existingResult.rows[0].count} records in state_registry_districts`);
    ...
    const csvPath = path.join(__dirname, '../data/processed/wy_superintendents.csv');
    const csv = fs.readFileSync(csvPath, 'utf8');
    const lines = csv.split('\n').slice(1).filter(l => l.trim());

    const records = [];
    for (const line of lines) {
      // Parse CSV: State,District,Superintendent,Title,Email,Phone,City,County
      const parts = line.split(',');
      if (parts.length < 7) continue;

      const [state, district, superintendent, title, email, phone, city, county] = parts;
      const { first, last } = parseName(superintendent);

      // Extract district ID (e.g., "#1" from "Albany County School District #1")
      const idMatch = district.match(/#(\d+)/);
      const districtId = idMatch ? `${county}-${idMatch[1]}` : county;

      records.push({
        state: 'WY',
        district_name: district,
        state_district_id: districtId,
    ...
    console.log(`Found ${ncesResult.rows.length} NCES districts for WY`);
    console.log('Note: Includes BOCES, institutions, and charters not in state directory');
    ...
    const importResult = await client.query(`
      ...
      VALUES (gen_random_uuid(), 'state_registry', 'Wyoming DOE Directory', $1, 'wy_superintendents.csv', $2,
              NOW(), 'fetch-load-wy.js', 'WY traditional county school districts. Gap of 13 = BOCES, institutions, charters.')
      RETURNING id
    ...
    for (const rec of records) {
      ...
      if (bestMatch) {
        await client.query(`
          ...
          VALUES (gen_random_uuid(), $1, $2, $3, $4, NOW(), 'fetch-load-wy.js', $5, $6)
        `, [
          bestMatch.nces_id,
          stateRegistryId,
          matchMethod,
    ...
    console.log(`\nWY Coverage: ${coverage.with_supt}/${coverage.total} (${coverage.pct}%)`);
