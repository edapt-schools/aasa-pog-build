/**
 * Alabama Superintendent Data - Load from CSV
 *
 * Source: School Superintendents of Alabama (SSA) Board
 * URL: https://www.ssaonline.org/board-of-directors
 * NCES: ~156 districts
 * CSV: 20 districts (SSA board members - confirmed superintendents)
 *
 * Note: Alabama's ALSDE directory requires login access.
 * This is a partial dataset from the SSA board members list.
 * Complete coverage would require manual district-by-district research.
 */
...
const SOURCE_URL = 'https://www.ssaonline.org/board-of-directors';
...
async function main() {
  ...
  try {
    ...
    if (parseInt(existingResult.rows[0].count) > 0) {
      console.log(`\nWARNING: AL already has ${existingResult.rows[0].count} records`);
    ...
    const csvPath = path.join(__dirname, '../data/processed/al_superintendents.csv');
    ...
    for (const line of lines) {
      ...
      const [state, district, superintendent] = parts;
      const { first, last } = parseName(superintendent);
      records.push({
        state: 'AL',
        district_name: district,
        state_district_id: district.replace(/Schools?|County|City|System|District/gi, '').trim(),
    ...
    console.log(`Found ${ncesResult.rows.length} NCES districts for AL`);
    ...
    const importResult = await client.query(`
      ...
      VALUES (gen_random_uuid(), 'state_registry', 'SSA Board of Directors', $1, 'al_superintendents.csv', $2,
              NOW(), 'fetch-load-al.js', 'AL partial - 20 districts from SSA board. ALSDE directory requires login.')
      RETURNING id
    ...
    for (const rec of records) {
      ...
      if (bestMatch) {
        await client.query(`
          ...
          VALUES (gen_random_uuid(), $1, $2, $3, $4, NOW(), 'fetch-load-al.js', $5, $6)
    ...
    const coverageResult = await client.query(`
      ...
      FROM national_registry WHERE state = 'AL'
    `);
    const coverage = coverageResult.rows[0];
    console.log(`\nAL Coverage: ${coverage.with_supt}/${coverage.total} (${coverage.pct}%)`);
    console.log(`Note: Partial data - ALSDE directory requires login for complete list.`);
