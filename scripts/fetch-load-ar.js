/**
 * Arkansas Superintendent Data - Load from CSV
 *
 * Source: Arkansas Department of Education Data Center
 * URL: https://adedata.arkansas.gov/statewide/reportlist/districts/SuperintendentContactList.aspx
 * NCES: ~260 districts
 * CSV: 252 districts (traditional + charter)
 *
 * Note: Excellent source with emails, phones, and addresses.
 * Data collected from paginated web interface (3 pages).
 */
...
const SOURCE_URL = 'https://adedata.arkansas.gov/statewide/reportlist/districts/SuperintendentContactList.aspx';
...
function normalizeDistrictName(name) {
  return name
    ...
    .replace(/public charter schools?/gi, '')
    .replace(/charter schools?/gi, '')
...
async function main() {
  ...
  try {
    ...
    if (parseInt(existingResult.rows[0].count) > 0) {
      console.log(`\nWARNING: AR already has ${existingResult.rows[0].count} records in state_registry_districts`);
    ...
    const csvPath = path.join(__dirname, '../data/processed/ar_superintendents.csv');
    ...
    for (const line of lines) {
      ...
      let stateDistrictId = district
        .replace(/School District|Public Charter|Charter|Academy/gi, '')
        .trim();
    ...
    console.log(`Found ${ncesResult.rows.length} NCES districts for AR`);
    ...
    const importResult = await client.query(`
      ...
      VALUES (gen_random_uuid(), 'state_registry', 'ADE Data Center Superintendent List', $1, 'ar_superintendents.csv', $2,
              NOW(), 'fetch-load-ar.js', 'AR districts from ADE Data Center. Includes traditional and charter schools.')
      RETURNING id
    ...
    for (const rec of records) {
      ...
      if (bestMatch) {
        await client.query(`
          ...
          VALUES (gen_random_uuid(), $1, $2, $3, $4, NOW(), 'fetch-load-ar.js', $5, $6)
        `, [
          bestMatch.nces_id,
          stateRegistryId,
          matchMethod,
    ...
    console.log(`\nAR Coverage: ${coverage.with_supt}/${coverage.total} (${coverage.pct}%)`);
