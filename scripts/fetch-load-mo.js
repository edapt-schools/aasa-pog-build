/**
 * Missouri Superintendent Data - Load from provided CSV
 *
 * Source: Missouri School District List.csv (user provided)
 * NCES: ~565 districts
 * CSV: ~561 records with superintendent data
 */
...
const SOURCE_URL = 'Missouri DESE';
...
function normalizeDistrictName(name) {
  return name
    .toLowerCase()
    .trim()
    .replace(/r-[ivxlcdm]+\s*/gi, '')  // Remove R-I, R-II, etc.
    .replace(/c-[0-9]+\s*/gi, '')      // Remove C-1, C-2, etc.
    .replace(/school district/gi, 'sd')
    .replace(/public schools?/gi, 'ps')
    .replace(/co\./gi, 'county')
    .replace(/[#.,()]/g, '')
    .replace(/\s+/g, ' ')
    .trim();
}
...
async function main() {
  ...
  try {
    ...
    if (parseInt(existingResult.rows[0].count) > 0) {
      console.log(`\nWARNING: MO already has ${existingResult.rows[0].count} records`);
      console.log('Skipping to avoid duplicates. Run delete-state.js MO first.');
      await client.end();
      return;
    }

    console.log('=== READING CSV ===');
    const csvPath = path.join(__dirname, '../Missouri School District List.csv');
    ...
    for (let i = 0; i < data.length; i++) {
      const row = data[i];
      if (!row.DNAME) continue;

      // Build full name from parts
      const firstName = row.SDFNAME?.trim() || '';
      const lastName = row.SDLNAME?.trim() || '';

      records.push({
        state: 'MO',
        district_name: row.DNAME?.trim(),
        state_district_id: row.CTYDIST || `MO-${i}`,
        administrator_first_name: firstName,
        administrator_last_name: lastName,
        administrator_email: row.DIEMAIL?.toLowerCase() || null,
        address: row.DADDRESS1,
        city: row.DCITY?.trim(),
        county: row.DCOUNTY?.trim(),
    ...
    console.log(`Found ${ncesResult.rows.length} NCES districts for MO`);
    ...
    const importResult = await client.query(`
      ...
      VALUES (gen_random_uuid(), 'state_registry', 'Missouri DESE', $1, 'Missouri School District List.csv', $2,
              NOW(), 'fetch-load-mo.js', 'MO districts from user-provided CSV. Includes superintendent names, emails, websites.')
      RETURNING id
    ...
    for (const rec of records) {
      const insertResult = await client.query(`
        ...
        (id, state, state_district_id, district_name, city, address,
        ...
        RETURNING id
      `, [rec.state, rec.state_district_id, rec.district_name, rec.city, rec.address,
      ...
      if (bestMatch) {
        await client.query(`
          ...
          VALUES (gen_random_uuid(), $1, $2, $3, $4, NOW(), 'fetch-load-mo.js', $5, $6)
    ...
    const coverageResult = await client.query(`
      ...
      FROM national_registry WHERE state = 'MO'
    `);
    const coverage = coverageResult.rows[0];
    console.log(`\nMO Coverage: ${coverage.with_supt}/${coverage.total} (${coverage.pct}%)`);
