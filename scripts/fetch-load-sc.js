/**
 * South Carolina Superintendent Data - Load from CSV
 *
 * Source: SC Association of School Administrators (SCASA)
 * URL: https://www.scasa.org/content.asp?contentid=221
 * NCES: ~85 districts (includes charter authorities, special schools)
 * CSV: 78 traditional districts + special entities
 *
 * Note: Emails not available from SCASA directory - phone only.
 * SC uses numbered districts within counties (e.g., Anderson 1-5).
 */
...
const SOURCE_URL = 'https://www.scasa.org/content.asp?contentid=221';
...
function normalizeDistrictName(name) {
  return name
    ...
    .replace(/county school/gi, '')
    .replace(/county schools/gi, '')
    .replace(/county public schools/gi, '')
...
async function main() {
  ...
  try {
    ...
    if (parseInt(existingResult.rows[0].count) > 0) {
      console.log(`\nWARNING: SC already has ${existingResult.rows[0].count} records in state_registry_districts`);
    ...
    const csvPath = path.join(__dirname, '../data/processed/sc_superintendents.csv');
    ...
    for (const line of lines) {
      ...
      let stateDistrictId = district
        .replace(/School District|Public Schools?|Schools?|County/gi, '')
        .trim();
    ...
    console.log(`Found ${ncesResult.rows.length} NCES districts for SC`);
    ...
    const importResult = await client.query(`
      ...
      VALUES (gen_random_uuid(), 'state_registry', 'SCASA Superintendent Directory', $1, 'sc_superintendents.csv', $2,
              NOW(), 'fetch-load-sc.js', 'SC districts from SCASA. Note: emails not available from this source.')
      RETURNING id
    ...
    for (const rec of records) {
      ...
      if (bestMatch) {
        await client.query(`
          ...
          VALUES (gen_random_uuid(), $1, $2, $3, $4, NOW(), 'fetch-load-sc.js', $5, $6)
        `, [
          bestMatch.nces_id,
          stateRegistryId,
          matchMethod,
    ...
    console.log(`\nSC Coverage: ${coverage.with_supt}/${coverage.total} (${coverage.pct}%)`);
