/**
 * Delaware Superintendent Data - Load from CSV
 *
 * Source: University of Delaware IPA Municipal Officials Directory
 * URL: https://www.udel.edu/academics/colleges/biden-school/research-public-service/ipa/people/delaware-municipal-officials/school-districts/
 * NCES: ~19 districts
 * CSV: 19 districts
 *
 * Note: Complete coverage of traditional public school districts.
...
const SOURCE_URL = 'https://www.udel.edu/academics/colleges/biden-school/research-public-service/ipa/people/delaware-municipal-officials/school-districts/';
...
function normalizeDistrictName(name) {
  return name
    ...
    .replace(/vocational technical/gi, 'vo-tech')
    .replace(/[#.,()]/g, '')
    .replace(/\s+/g, ' ')
    .trim();
}
...
function parseName(fullName) {
  ...
  let cleaned = fullName
    ...
    .replace(/,?\s*(Jr\.?|Sr\.?|III|II|IV|Ed\.D\.)$/gi, '')
    .trim();
...
async function main() {
  ...
  try {
    ...
    if (parseInt(existingResult.rows[0].count) > 0) {
      console.log(`\nWARNING: DE already has ${existingResult.rows[0].count} records`);
    ...
    const csvPath = path.join(__dirname, '../data/processed/de_superintendents.csv');
    ...
    for (const line of lines) {
      ...
      records.push({
        ...
        state_district_id: district.replace(/School District|Consolidated/gi, '').trim(),
    ...
    console.log(`Found ${ncesResult.rows.length} NCES districts for DE`);
    ...
    const importResult = await client.query(`
      ...
      VALUES (gen_random_uuid(), 'state_registry', 'UD IPA Municipal Officials Directory', $1, 'de_superintendents.csv', $2,
              NOW(), 'fetch-load-de.js', 'DE traditional districts from UD IPA. Complete with emails and phones.')
      RETURNING id
    ...
    for (const rec of records) {
      ...
      if (bestMatch) {
        await client.query(`
          ...
          VALUES (gen_random_uuid(), $1, $2, $3, $4, NOW(), 'fetch-load-de.js', $5, $6)
    ...
    const coverageResult = await client.query(`
      ...
      FROM national_registry WHERE state = 'DE'
    `);
    const coverage = coverageResult.rows[0];
    console.log(`\nDE Coverage: ${coverage.with_supt}/${coverage.total} (${coverage.pct}%)`);
