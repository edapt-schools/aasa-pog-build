/**
 * Nevada Superintendent Data - Load from CSV
 *
 * Source: Nevada DOE School Directory
 * URL: https://doe.nv.gov/
 * NCES: 20 districts (includes charter authority, corrections)
 * CSV: 17 county school districts
 *
 * Note: The 3-district gap consists of:
 * - Correctional School District (prison education)
 * - Davidson Academy (specialized gifted school)
 * - State Sponsored Charter Schools (charter authority)
 * All 17 county districts are complete.
 */
...
const SOURCE_URL = 'https://doe.nv.gov/';
...
async function main() {
  ...
  try {
    ...
    if (parseInt(existingResult.rows[0].count) > 0) {
      console.log(`\nWARNING: NV already has ${existingResult.rows[0].count} records in state_registry_districts`);
    ...
    const csvPath = path.join(__dirname, '../data/processed/nv_superintendents.csv');
    ...
    for (const line of lines) {
      ...
      // Extract county name as state_district_id
      const countyMatch = district.match(/^(\w+(?:\s+\w+)?)\s+County/i);
      const stateDistrictId = countyMatch ? countyMatch[1] : district.replace(/\s+School District$/i, '');
    ...
    console.log(`Found ${ncesResult.rows.length} NCES districts for NV`);
    console.log('Note: Includes charter authority, corrections, and specialized schools');
    ...
    const importResult = await client.query(`
      ...
      VALUES (gen_random_uuid(), 'state_registry', 'Nevada DOE Directory', $1, 'nv_superintendents.csv', $2,
              NOW(), 'fetch-load-nv.js', 'NV 17 county school districts. Gap of 3 = charter authority, corrections, specialized.')
      RETURNING id
    ...
    for (const rec of records) {
      ...
      if (bestMatch) {
        await client.query(`
          ...
          VALUES (gen_random_uuid(), $1, $2, $3, $4, NOW(), 'fetch-load-nv.js', $5, $6)
        `, [
          bestMatch.nces_id,
          stateRegistryId,
          matchMethod,
    ...
    console.log(`\nNV Coverage: ${coverage.with_supt}/${coverage.total} (${coverage.pct}%)`);
