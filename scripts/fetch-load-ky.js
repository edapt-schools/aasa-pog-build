/**
 * Kentucky Superintendent Data - Load from provided CSV
 *
 * Source: KY supts.csv (user provided)
 * NCES: ~177 districts
 * CSV: ~177 superintendents
 */
...
async function main() {
  ...
  try {
    ...
    if (parseInt(existingResult.rows[0].count) > 0) {
      console.log(`\nWARNING: KY already has ${existingResult.rows[0].count} records`);
      console.log('Skipping to avoid duplicates. Run delete-state.js KY first.');
      await client.end();
      return;
    }

    console.log('=== READING CSV ===');
    const csvPath = path.join(__dirname, '../KY supts.csv');
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      records.push({
        ...
        state_district_id: row['District Code'] || `KY-${i}`,
        administrator_first_name: row['Superintendent First Name'] || '',
        administrator_last_name: row['Superintendent Last Name'] || '',
        phone: row.Phone,
        address: row['Address Line 1'],
        website_url: row['District Website'] || null
      });
    }

    console.log(`Parsed ${records.length} records`);
    console.log(`With website: ${records.filter(r => r.website_url).length}`);
    ...
    console.log(`Found ${ncesResult.rows.length} NCES districts for KY`);
    ...
    const importResult = await client.query(`
      ...
      VALUES (gen_random_uuid(), 'state_registry', 'Kentucky KDE', $1, 'KY supts.csv', $2,
              NOW(), 'fetch-load-ky.js', 'KY superintendents from KDE. Includes names, phones, websites.')
      RETURNING id
    ...
    for (const rec of records) {
      ...
      if (bestMatch) {
        await client.query(`
          ...
          VALUES (gen_random_uuid(), $1, $2, $3, $4, NOW(), 'fetch-load-ky.js', $5, $6)
    ...
    const coverageResult = await client.query(`
      ...
      FROM national_registry WHERE state = 'KY'
    `);
    const coverage = coverageResult.rows[0];
    console.log(`\nKY Coverage: ${coverage.with_supt}/${coverage.total} (${coverage.pct}%)`);
