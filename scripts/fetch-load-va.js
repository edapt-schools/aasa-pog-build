/**
 * Virginia Superintendent Data - Load from CSV
 *
 * Source: Virginia Department of Education - Division Superintendents
 * URL: https://www.va-doeapp.com/SuperintendentBySchoolDivisions.aspx
 * NCES: ~132 divisions
 * CSV: 126 divisions (8 positions currently vacant)
 *
 * Note: Virginia uses "divisions" instead of "districts".
 * Emails not available from VA DOE directory - only phone numbers.
 */
...
const SOURCE_URL = 'https://www.va-doeapp.com/SuperintendentBySchoolDivisions.aspx';
...
async function main() {
  ...
  try {
    ...
    if (parseInt(existingResult.rows[0].count) > 0) {
      console.log(`\nWARNING: VA already has ${existingResult.rows[0].count} records in state_registry_districts`);
    ...
    const csvPath = path.join(__dirname, '../data/processed/va_superintendents.csv');
    ...
    for (const line of lines) {
      ...
      let stateDistrictId = district
        .replace(/Public Schools?|County|City/gi, '')
        .trim();
    ...
    console.log(`Found ${ncesResult.rows.length} NCES districts for VA`);
    ...
    const importResult = await client.query(`
      ...
      VALUES (gen_random_uuid(), 'state_registry', 'VA DOE Division Superintendents', $1, 'va_superintendents.csv', $2,
              NOW(), 'fetch-load-va.js', 'VA divisions from VA DOE. 8 vacant positions excluded. Note: emails not available.')
      RETURNING id
    ...
    for (const rec of records) {
      ...
      if (bestMatch) {
        await client.query(`
          ...
          VALUES (gen_random_uuid(), $1, $2, $3, $4, NOW(), 'fetch-load-va.js', $5, $6)
        `, [
          bestMatch.nces_id,
          stateRegistryId,
          matchMethod,
    ...
    console.log(`\nVA Coverage: ${coverage.with_supt}/${coverage.total} (${coverage.pct}%)`);
