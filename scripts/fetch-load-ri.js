/**
 * Rhode Island Superintendent Data - Load from CSV
 *
 * Source: Rhode Island DOE
 * NCES: ~67 districts
 * CSV: 36 traditional public school districts
 */
...
const SOURCE_URL = 'https://ride.ri.gov/';
...
async function main() {
  ...
  try {
    ...
    if (parseInt(existingResult.rows[0].count) > 0) {
      console.log(`\nWARNING: RI already has ${existingResult.rows[0].count} records`);
    ...
    const csvPath = path.join(__dirname, '../data/processed/ri_superintendents.csv');
    const csv = fs.readFileSync(csvPath, 'utf8');
    const lines = csv.split('\n').slice(1).filter(l => l.trim());

    const records = [];
    for (const line of lines) {
      // State,District,Superintendent,Title,Email,Phone,City
      const parts = line.split(',');
      if (parts.length < 6) continue;

      const [state, district, superintendent, title, email, phone, city] = parts;
      ...
      records.push({
        ...
        state_district_id: district.substring(0, 50),
        city: city,
    ...
    console.log(`Found ${ncesResult.rows.length} NCES districts for RI`);
    ...
    const importResult = await client.query(`
      ...
      VALUES (gen_random_uuid(), 'state_registry', 'Rhode Island DOE', $1, 'ri_superintendents.csv', $2,
              NOW(), 'fetch-load-ri.js', 'RI traditional public school districts. Gap = charter LEAs not included.')
      RETURNING id
    ...
    for (const rec of records) {
      ...
      if (bestMatch) {
        await client.query(`
          ...
          VALUES (gen_random_uuid(), $1, $2, $3, $4, NOW(), 'fetch-load-ri.js', $5, $6)
    ...
    const coverageResult = await client.query(`
      ...
      FROM national_registry WHERE state = 'RI'
    `);
    const coverage = coverageResult.rows[0];
    console.log(`\nRI Coverage: ${coverage.with_supt}/${coverage.total} (${coverage.pct}%)`);
