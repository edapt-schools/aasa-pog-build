/**
 * Pennsylvania Superintendent Data - Load from provided CSV
 *
 * Source: PA List - Sheet1 (1).csv (user provided)
 * NCES: ~784 districts
 * CSV: ~672 records (districts + charters)
 */
...
const SOURCE_URL = 'PA Department of Education';
...
function normalizeDistrictName(name) {
  return name
    ...
    .replace(/charter school/gi, 'cs')
    .replace(/cyber charter school/gi, 'ccs')
    .replace(/[#.,()]/g, '')
    .replace(/\s+/g, ' ')
    .trim();
}
...
function parseAdministrator(adminStr) {
  // Format: "Dr Matthew Flannery , Chief Administrator (CS Only)"
  // or "Dr Jeffrey S Fecher , Superintendent"
  if (!adminStr) return { first: '', last: '', title: '' };
  ...
  if (nameParts.length === 1) {
    return { first: '', last: nameParts[0], title: titlePart };
  }
  if (nameParts.length === 2) {
    return { first: nameParts[0], last: nameParts[1], title: titlePart };
  }
  // Handle middle name/initial
  return { first: nameParts[0], last: nameParts.slice(1).join(' '), title: titlePart };
}
...
async function main() {
  ...
  try {
    ...
    if (parseInt(existingResult.rows[0].count) > 0) {
      console.log(`\nWARNING: PA already has ${existingResult.rows[0].count} records`);
      console.log('Skipping to avoid duplicates. Run delete-state.js PA first.');
      await client.end();
      return;
    }

    console.log('=== READING CSV ===');
    const csvPath = path.join(__dirname, '../PA List - Sheet1 (1).csv');
    const content = fs.readFileSync(csvPath, 'utf-8');
    const data = parseCSV(content);

    // Filter for open districts only
    const openDistricts = data.filter(r => r.Status === 'Open');
    console.log(`Total records: ${data.length}`);
    console.log(`Open districts: ${openDistricts.length}`);

    const records = [];
    for (const row of openDistricts) {
      const admin = parseAdministrator(row['Primary Administrator']);

      records.push({
        state: 'PA',
        district_name: row['Institution Name'],
        state_district_id: row['Administrative Unit (AUN)'],
        administrator_first_name: admin.first,
        administrator_last_name: admin.last,
        administrator_title: admin.title,
        phone: row.Phone,
        city: row.City,
        county: row.County,
        website_url: row['Web Address'] !== 'N/A' ? row['Web Address'] : null
      });
    }

    console.log(`Parsed ${records.length} records`);
    console.log(`With administrator: ${records.filter(r => r.administrator_last_name).length}`);
    ...
    console.log(`Found ${ncesResult.rows.length} NCES districts for PA`);
    ...
    const importResult = await client.query(`
      ...
      VALUES (gen_random_uuid(), 'state_registry', 'PA Department of Education', $1, 'PA List - Sheet1 (1).csv', $2,
              NOW(), 'fetch-load-pa.js', 'PA districts from user-provided CSV. Includes superintendent names, phones, websites.')
      RETURNING id
    ...
    for (const rec of records) {
      const insertResult = await client.query(`
        ...
        RETURNING id
      `, [rec.state, rec.state_district_id, rec.district_name, rec.city, rec.phone,
      ...
      if (bestMatch) {
        await client.query(`
          ...
          VALUES (gen_random_uuid(), $1, $2, $3, $4, NOW(), 'fetch-load-pa.js', $5, $6)
    ...
    const coverageResult = await client.query(`
      ...
      FROM national_registry WHERE state = 'PA'
    `);
    const coverage = coverageResult.rows[0];
    console.log(`\nPA Coverage: ${coverage.with_supt}/${coverage.total} (${coverage.pct}%)`);
