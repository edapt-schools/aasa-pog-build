/**
 * New Mexico Superintendent Data - Load from CSV
 *
 * Source: CES Client New Mexico Superintendents Directory
 * URL: https://cesclient.sks.com/DirectoryDetail.aspx?ID=1
 * NCES: ~89 districts
 * CSV: 87 districts
 *
 * Note: Complete coverage of New Mexico public school districts.
...
const SOURCE_URL = 'https://cesclient.sks.com/DirectoryDetail.aspx?ID=1';
...
function normalizeDistrictName(name) {
  return name
    ...
    .replace(/consolidated schools/gi, '')
    .replace(/independent schools/gi, '')
...
async function main() {
  ...
  try {
    ...
    if (parseInt(existingResult.rows[0].count) > 0) {
      console.log(`\nWARNING: NM already has ${existingResult.rows[0].count} records`);
    ...
    const csvPath = path.join(__dirname, '../data/processed/nm_superintendents.csv');
    ...
    for (const line of lines) {
      ...
      records.push({
        ...
        state_district_id: district.replace(/Public Schools|Municipal Schools|Consolidated Schools|Independent Schools|School District|County Schools/gi, '').trim(),
    ...
    console.log(`Found ${ncesResult.rows.length} NCES districts for NM`);
    ...
    const importResult = await client.query(`
      ...
      VALUES (gen_random_uuid(), 'state_registry', 'CES Client NM Superintendents Directory', $1, 'nm_superintendents.csv', $2,
              NOW(), 'fetch-load-nm.js', 'NM districts from CES directory. Complete with emails and phones.')
      RETURNING id
    ...
    for (const rec of records) {
      ...
      if (bestMatch) {
        await client.query(`
          ...
          VALUES (gen_random_uuid(), $1, $2, $3, $4, NOW(), 'fetch-load-nm.js', $5, $6)
    ...
    const coverageResult = await client.query(`
      ...
      FROM national_registry WHERE state = 'NM'
    `);
    const coverage = coverageResult.rows[0];
    console.log(`\nNM Coverage: ${coverage.with_supt}/${coverage.total} (${coverage.pct}%)`);
