/**
 * North Dakota Superintendent Data - Load from CSV
 *
 ...
 * CSV: 164 districts
 *
 * Note: Excellent source with emails and phones from official DPI Excel file.
 * ND uses numbered districts (e.g., "Valley City 2", "Hettinger 13").
 */
...
const SOURCE_URL = 'https://www.nd.gov/dpi/data';
...
function normalizeDistrictName(name) {
  return name
    ...
    .replace(/schools/gi, '')
    .replace(/county/gi, '')
    .replace(/\d+$/g, '') // Remove trailing district numbers
    .replace(/[#.,()]/g, '')
    .replace(/\s+/g, ' ')
    .trim();
}
...
async function main() {
  ...
  try {
    ...
    if (parseInt(existingResult.rows[0].count) > 0) {
      console.log(`\nWARNING: ND already has ${existingResult.rows[0].count} records in state_registry_districts`);
    ...
    const csvPath = path.join(__dirname, '../data/processed/nd_superintendents.csv');
    ...
    for (const line of lines) {
      ...
      // Extract district ID (number at end)
      let stateDistrictId = district.match(/\d+$/)?.[0] || district;
    ...
    console.log(`Found ${ncesResult.rows.length} NCES districts for ND`);
    ...
    const importResult = await client.query(`
      ...
      VALUES (gen_random_uuid(), 'state_registry', 'ND DPI Administrator Directory', $1, 'nd_superintendents.csv', $2,
              NOW(), 'fetch-load-nd.js', 'ND districts from DPI Excel download. Includes emails and phones.')
      RETURNING id
    ...
    for (const rec of records) {
      ...
      if (bestMatch) {
        await client.query(`
          ...
          VALUES (gen_random_uuid(), $1, $2, $3, $4, NOW(), 'fetch-load-nd.js', $5, $6)
        `, [
          bestMatch.nces_id,
          stateRegistryId,
          matchMethod,
    ...
    console.log(`\nND Coverage: ${coverage.with_supt}/${coverage.total} (${coverage.pct}%)`);
