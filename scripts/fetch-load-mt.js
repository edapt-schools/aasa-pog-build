/**
 * Montana Superintendent Data - Load from provided CSV
 *
 * Source: MT List of Districts and Superintendents - Sheet1.csv (user provided)
 * NCES: ~474 districts
 * CSV: ~273 records
 */
...
async function main() {
  ...
  try {
    ...
    if (parseInt(existingResult.rows[0].count) > 0) {
      console.log(`\nWARNING: MT already has ${existingResult.rows[0].count} records`);
      console.log('Skipping to avoid duplicates. Run delete-state.js MT first.');
      await client.end();
      return;
    }

    console.log('=== READING CSV ===');
    const csvPath = path.join(__dirname, '../MT List of Districts and Superintendents - Sheet1.csv');
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      records.push({
        state: 'MT',
        district_name: row['System Name'],
        state_district_id: row['SS ']?.trim() || `MT-${i}`,  // Generate ID if missing
        administrator_first_name: row.First || '',
        administrator_last_name: row.Last || '',
        administrator_email: row['Staff E mail']?.toLowerCase() || null,
        phone: row.Phone,
        city: row.City,
        county: row['County Name'],
        website_url: extractWebsite(row['Staff E mail'])
      });
    }
    ...
    console.log(`Found ${ncesResult.rows.length} NCES districts for MT`);
    ...
    const importResult = await client.query(`
      ...
      VALUES (gen_random_uuid(), 'state_registry', 'Montana OPI', $1, 'MT List of Districts and Superintendents - Sheet1.csv', $2,
              NOW(), 'fetch-load-mt.js', 'MT districts from user-provided CSV. Includes superintendent names, emails, phones.')
      RETURNING id
    ...
    for (const rec of records) {
      ...
      if (bestMatch) {
        await client.query(`
          ...
          VALUES (gen_random_uuid(), $1, $2, $3, $4, NOW(), 'fetch-load-mt.js', $5, $6)
    ...
    const coverageResult = await client.query(`
      ...
      FROM national_registry WHERE state = 'MT'
    `);
    const coverage = coverageResult.rows[0];
    console.log(`\nMT Coverage: ${coverage.with_supt}/${coverage.total} (${coverage.pct}%)`);
