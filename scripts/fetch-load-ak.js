/**
 * Alaska Superintendent Data - Load from CSV
 *
 * Source: Alaska Superintendents Association Contact Sheet 2024-2025
 * URL: https://alaskaacsa.org/wp-content/uploads/2024-2025-Alaska-Superintendents-Association-Contact-Sheet-updated-10.28.24.pdf
 * NCES: ~53 districts
 * CSV: 53 districts
 *
 * Note: Complete coverage of Alaska school districts including borough, city, and REAA districts.
 * Excellent source with emails and phone numbers.
 */
...
const SOURCE_URL = 'https://alaskaacsa.org/wp-content/uploads/2024-2025-Alaska-Superintendents-Association-Contact-Sheet-updated-10.28.24.pdf';
...
function normalizeDistrictName(name) {
  return name
    ...
    .replace(/borough school/gi, '')
    .replace(/city school/gi, '')
    .replace(/public school/gi, '')
    .replace(/area school/gi, '')
    .replace(/region school/gi, '')
...
async function main() {
  ...
  try {
    ...
    if (parseInt(existingResult.rows[0].count) > 0) {
      console.log(`\nWARNING: AK already has ${existingResult.rows[0].count} records`);
    ...
    const csvPath = path.join(__dirname, '../data/processed/ak_superintendents.csv');
    ...
    for (const line of lines) {
      ...
      records.push({
        ...
        state_district_id: district.replace(/School District|Borough|City|Public|Area|Region/gi, '').trim(),
    ...
    console.log(`Found ${ncesResult.rows.length} NCES districts for AK`);
    ...
    const importResult = await client.query(`
      ...
      VALUES (gen_random_uuid(), 'state_registry', 'Alaska Superintendents Association Contact Sheet', $1, 'ak_superintendents.csv', $2,
              NOW(), 'fetch-load-ak.js', 'AK districts from ASA 2024-2025 contact sheet. Complete with emails and phones.')
      RETURNING id
    ...
    for (const rec of records) {
      ...
      if (bestMatch) {
        await client.query(`
          ...
          VALUES (gen_random_uuid(), $1, $2, $3, $4, NOW(), 'fetch-load-ak.js', $5, $6)
    ...
    const coverageResult = await client.query(`
      ...
      FROM national_registry WHERE state = 'AK'
    `);
    const coverage = coverageResult.rows[0];
    console.log(`\nAK Coverage: ${coverage.with_supt}/${coverage.total} (${coverage.pct}%)`);
