/**
 * Washington Superintendent Data - Load from provided CSV
 *
 * Source: WA Supt List - Sheet1.csv (user provided)
 * NCES: ~341 districts
 * CSV: ~318 superintendents
 */
...
const SOURCE_URL = 'Washington OSPI';
...
async function main() {
  ...
  try {
    ...
    if (parseInt(existingResult.rows[0].count) > 0) {
      console.log(`\nWARNING: WA already has ${existingResult.rows[0].count} records`);
      console.log('Skipping to avoid duplicates. Run delete-state.js WA first.');
      await client.end();
      return;
    }

    console.log('=== READING CSV ===');
    const csvPath = path.join(__dirname, '../WA Supt List - Sheet1.csv');
    ...
    for (let i = 0; i < data.length; i++) {
      const row = data[i];
      if (!row['Local Education Agency']) continue;

      const { first, last } = parseName(row['Administrator Name']);

      records.push({
        state: 'WA',
        district_name: row['Local Education Agency'],
        state_district_id: row['LEA Code'] || `WA-${i}`,
    ...
    console.log(`Found ${ncesResult.rows.length} NCES districts for WA`);
    ...
    const importResult = await client.query(`
      ...
      VALUES (gen_random_uuid(), 'state_registry', 'Washington OSPI', $1, 'WA Supt List - Sheet1.csv', $2,
              NOW(), 'fetch-load-wa.js', 'WA superintendents from OSPI list. Includes names, emails, phones.')
      RETURNING id
    ...
    for (const rec of records) {
      ...
      if (bestMatch) {
        await client.query(`
          ...
          VALUES (gen_random_uuid(), $1, $2, $3, $4, NOW(), 'fetch-load-wa.js', $5, $6)
    ...
    const coverageResult = await client.query(`
      ...
      FROM national_registry WHERE state = 'WA'
    `);
    const coverage = coverageResult.rows[0];
    console.log(`\nWA Coverage: ${coverage.with_supt}/${coverage.total} (${coverage.pct}%)`);
