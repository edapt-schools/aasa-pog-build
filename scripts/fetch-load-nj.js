/**
 * New Jersey Superintendent Data - Load from CSV
 *
 * Source: NJDOE District Directory
 * URL: https://homeroom6.doe.state.nj.us/directory/
 * NCES: ~695 districts
 * CSV: 559 districts
 *
 * Note: Emails and phones not available from public directory.
 * NJ uses county-based organization with many small districts.
 */
...
const SOURCE_URL = 'https://homeroom6.doe.state.nj.us/directory/';
...
function normalizeDistrictName(name) {
  return name
    ...
    .replace(/regional school/gi, '')
    .replace(/regional/gi, '')
    .replace(/township|twp/gi, '')
    .replace(/borough|boro/gi, '')
    .replace(/city/gi, '')
    .replace(/public schools?/gi, '')
    .replace(/vocational/gi, '')
    .replace(/county special service/gi, '')
    .replace(/county/gi, '')
    .replace(/[#.,()]/g, '')
    .replace(/\s+/g, ' ')
    .trim();
...
async function main() {
  ...
  try {
    ...
    if (parseInt(existingResult.rows[0].count) > 0) {
      console.log(`\nWARNING: NJ already has ${existingResult.rows[0].count} records`);
      console.log('Skipping to avoid duplicates. Run delete-state.js NJ first.');
      await client.end();
      return;
    }

    console.log('=== READING CSV ===');
    const csvPath = path.join(__dirname, '../data/processed/nj_superintendents.csv');
    const csv = fs.readFileSync(csvPath, 'utf8');
    const lines = csv.split('\n').slice(1).filter(l => l.trim());

    const records = [];
    for (const line of lines) {
      // Parse CSV - format: state,state_district_id,district_name,administrator_first_name,administrator_last_name,county
      ...
      const [state, stateDistrictId, districtName, firstName, lastName, county] = parts;

      // Skip entries with no real superintendent name
      if (!firstName || firstName.toLowerCase() === 'shared services' || !lastName || lastName.match(/^\d+$/)) {
        continue;
      }
    ...
    console.log(`Found ${ncesResult.rows.length} NCES districts for NJ`);
    ...
    const importResult = await client.query(`
      ...
      VALUES (gen_random_uuid(), 'state_registry', 'NJDOE District Directory', $1, 'nj_superintendents.csv', $2,
              NOW(), 'fetch-load-nj.js', 'NJ districts. Note: emails/phones not available from this source.')
      RETURNING id
    ...
    for (const rec of records) {
      ...
      if (bestMatch) {
        await client.query(`
          ...
          VALUES (gen_random_uuid(), $1, $2, $3, $4, NOW(), 'fetch-load-nj.js', $5, $6)
    ...
    const coverageResult = await client.query(`
      ...
      FROM national_registry WHERE state = 'NJ'
    `);
    const coverage = coverageResult.rows[0];
    console.log(`\nNJ Coverage: ${coverage.with_supt}/${coverage.total} (${coverage.pct}%)`);
